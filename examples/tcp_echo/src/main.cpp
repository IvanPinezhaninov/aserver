...
class TcpEcho final : public server_worker<ip::tcp> {
public:
  ...
  ~TcpEcho() { std::cout << "Worker destroyed" << std::endl; }
...
int main(int, char **)
{
  try {
    ...
    auto stop = server.bind_endpoint<TcpEcho>({ip::tcp::v4(), 12345}, "Hello! I'm an echo server.");
