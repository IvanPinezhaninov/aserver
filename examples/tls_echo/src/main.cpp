...
public:
                  ...
                  net::yield_context yield) override
  {
    ...
    try {
      ...
      os << m_greetings << '\n';
      net::async_write(stream, buffer, net::bind_cancellation_slot(m_signal.slot(), yield));
      ...
      stream.lowest_layer().shutdown(ip::tcp::socket::shutdown_both);
      stream.lowest_layer().close();
    } catch (...) {
      LOG(info) << "Receiver abnormally closed" << logEndpoint(endpoint);
      throw;
    }
