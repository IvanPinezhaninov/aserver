...
class HttpsEcho final : public aserver::receiver<ip::tcp> {
public:
                  ...
                  net::yield_context yield) final
  {
    ...
    try {
      ...
      stream.async_handshake(ssl::stream_base::server, net::bind_cancellation_slot(m_signal.slot(), yield));
      ...
      while (true) {
        try {
          ...
          http::async_read(stream, buffer, parser, net::bind_cancellation_slot(m_signal.slot(), yield));
          const auto& req = parser.get();
          http::async_write(stream, echoResponse(req), net::bind_cancellation_slot(m_signal.slot(), yield));
          ...
          if (e.code() == beast::error::timeout) {
            LOG(info) << "Client disconnected due to a timeout" << logEndpoint(endpoint);
            break;
          }
          throw;
        }
      }
      closeStream(stream, m_signal.slot(), yield);
    } catch (...) {
      closeStream(stream, m_signal.slot(), yield);
