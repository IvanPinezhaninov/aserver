...
protected:
  ...
  void startServer(typename Receiver::protocol_type::endpoint endpoint, Args&&... args)
  {
    auto tuple = std::make_tuple(std::forward<Args>(args)...);
    m_thread = std::thread{[this, endpoint = std::move(endpoint), tuple = std::move(tuple)]() mutable {
      std::apply(
          [this, endpoint = std::move(endpoint)](auto&&... args) {
            m_server.bind_receiver<Receiver>(endpoint, std::forward<decltype(args)>(args)...);
            m_server.run();
          },
          std::move(tuple));
    }};
