...
public:
  ...
  virtual void operator()(boost::asio::io_context& ioc, typename protocol_type::socket socket,
                          boost::asio::cancellation_slot cancel, boost::asio::yield_context yield) = 0;
};
...
public:
  ...
  public:
    cancel_token(std::weak_ptr<cancel_pool> storage, iterator_type it)
    ...
    {
      if (auto storage = m_storage.lock()) return m_it->signal.slot();
      return {};
    }
    ...
    return net::async_compose<CompletionToken, void(cancel_token)>(
        [self = shared_from_this(), ex, token](auto& compose) {
          net::dispatch(ex, [self = std::move(self), &compose, ex, token]() {
            compose.complete(cancel_token{self, self->m_entries.emplace(self->m_entries.end(), token)});
          });
        },
        token);
  }
...
private:
  ...
  {
    namespace net = boost::asio;
    net::dispatch(m_yield.get_executor(), [self = shared_from_this(), it] {
  ...
  {
    boost::asio::dispatch(m_yield.get_executor(), [self = shared_from_this(), it] {
      switch (it->state) {
      case entry::state::idle:
        self->m_entries.erase(it);
        break;
...
public:
      ...
      net::spawn(
          ...
          [this, acceptor = std::move(acceptor), err_handler, stop_signal,
           args = std::forward_as_tuple(std::forward<Args>(args)...)](net::yield_context yield) mutable {
            ...
            try {
              do_accept<Receiver>(std::move(acceptor), err_handler, stop_signal.slot(), yield, std::move(args));
            } catch (...) {
      ...
      net::spawn(
          ...
          [this, socket = std::move(socket), err_handler, stop_signal,
           args = std::forward_as_tuple(std::forward<Args>(args)...)](net::yield_context yield) mutable {
            ...
            track_stop_signal(stop_signal);
            try {
              do_receive<Receiver>(std::move(socket), err_handler, stop_signal.slot(), yield, std::move(args));
            } catch (...) {
              err_handler(std::current_exception());
            }
            untrack_coro();
  ...
  void run()
  {
    ...
    m_work_guard.emplace(boost::asio::make_work_guard(m_ioc));
...
private:
    ...
    try {
      try {
        ...
        if (slot.is_connected()) {
          slot.assign([ex = yield.get_executor(), &running, &signal, &slot_called](net::cancellation_type) {
            auto exp = false;
            if (slot_called.compare_exchange_strong(exp, true, std::memory_order_relaxed, std::memory_order_relaxed)) {
      ...
      while (running) {
        try {
          ...
          acceptor.async_accept(socket, net::bind_cancellation_slot(signal.slot(), yield));
          ...
          if (e.code() != net::error::invalid_argument) err_handler(std::current_exception());
          continue;
        }
      }
  ...
  {
    untrack_pending_coro();
    std::make_from_tuple<Receiver>(std::move(args))(m_ioc, std::move(socket), slot, yield);
  }
