...
public:
  ...
  server_worker(const server_worker &) = default;
  server_worker(server_worker &&) = default;
  server_worker &operator=(const server_worker &) = default;
  server_worker &operator=(server_worker &&) = default;
  virtual ~server_worker() = default;
...
public:
  ...
  stop_signal bind_endpoint(const typename Worker::protocol_type::endpoint &endpoint, Args &&...args)
  {
    namespace net = boost::asio;
    stop_signal stop;
    if constexpr (has_acceptor<typename Worker::protocol_type>::value) {
      typename Worker::protocol_type::acceptor acceptor{net::make_strand(m_ioc), endpoint};
      net::spawn(m_ioc, [this, acceptor = std::move(acceptor), stop,
        ...
        do_accept<Worker>(std::move(acceptor), std::move(stop), yield, std::move(args));
      });
    } else {
      typename Worker::protocol_type::socket socket{net::make_strand(m_ioc), endpoint};
      net::spawn(m_ioc, [this, socket = std::move(socket), stop,
                         args = std::make_tuple(std::forward<Args>(args)...)](net::yield_context yield) mutable {
        start_worker<Worker>(std::move(socket), std::move(stop), yield, std::move(args));
      });
    }
    return stop;
  }
  ...
  void run()
  {
    ...
    if (m_on_run)
      boost::asio::post(m_ioc, m_on_run.value());
...
private:
  template<typename Worker, typename... Args, typename Acceptor = typename Worker::protocol_type::acceptor>
  void do_accept(Acceptor acceptor, stop_signal stop, boost::asio::yield_context yield, std::tuple<Args...> args)
  {
    ...
    while (!stopped) {
      typename Worker::protocol_type::socket socket{net::make_strand(m_ioc)};
  ...
  template<typename Worker, typename... Args, typename Socket = typename Worker::protocol_type::socket>
  void start_worker(Socket socket, stop_signal stop, boost::asio::yield_context yield, std::tuple<Args...> args)
  {
    std::make_from_tuple<Worker>(std::move(args))(m_ioc, std::move(socket), std::move(stop), yield);
  }
